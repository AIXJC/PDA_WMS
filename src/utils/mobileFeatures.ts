...
/**
 * Request camera stream (web fallback).
...
export const requestCamera = async (facingMode: 'user' | 'environment' = 'environment'): Promise<MediaStream> => {
  if (isInWebView()) {
    // If running inside native WebView, prefer native bridge if available
    try {
      const granted = await callNative('requestCameraPermission');
      if (!granted) throw new Error('camera permission denied by native');
    } catch (err) {
      // fallback to web
    }
  }
